...
export class LlmService {
	...
	async callAuditorLLM(
		...
		const chainAuditData = await ChainAuditSchema.findOne({
			id: activeChainId,
		});
		...
		const auditorModel =
			chainAuditData.auditorModel || 'ft:gpt-3.5-turbo-1106:personal::8Pw67TV2';
		...
		try {
			...
			if (!validateSchema(parsedResponse)) {
				...
				return this.callAuditorLLM(code, maxTries - 1);
			}
			// schema is valid
			return {
				audits: parsedResponse.audits as AuditorResponse['audits'],
				success: true,
			};
		} catch (error) {
			return this.callAuditorLLM(code, maxTries - 1);
